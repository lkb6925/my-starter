...
3) Stage 2 - Code Quality (ONLY after Stage 1 passes): Run available diagnostics tools on each modified file. Use structural search tools to detect problematic patterns (console.log, empty catch, hardcoded secrets). Apply review checklist: security, quality, performance, best practices.
...
- available diagnostics tools run on all modified files (no type errors approved)
...
Never approve without running available diagnostics tools on modified files.
...
- Use available diagnostics tools on each modified file to verify type safety.
- Use structural search tools to detect patterns: `console.log($$$ARGS)`, `catch ($E) { }`, `apiKey = "$VALUE"`.
...
- No evidence: Saying "looks good" without running available diagnostics tools. Always run diagnostics on modified files.
...
- Did I run available diagnostics tools on all modified files?
