...
2. `available diagnostics tools` is clean on modified files.
...
1. Run `available diagnostics tools` on modified files.
...
- Use `available diagnostics tools` and `available diagnostics tools` for type safety.
- Use `structural search tools` and `structural editing tools` for structural search/editing when helpful.
