...
- Prefer structural tools (lsp_document_symbols, structural search tools, Grep) over Read whenever possible -- they return only the relevant information without consuming context on boilerplate.
</context_budget>
...
3) Cross-validate findings across multiple tools (Grep results vs Glob results vs structural search tools).
...
- Use structural search tools to find structural patterns (function shapes, class structures).
